...
class ConfigBox(VBox):
    ...
    def call_widget_generator(self, option, option_key, value,
                              default):  # noqa: C901
        ...
        if option_type == "choice":
            self.generate_combobox(option_key, option["choices"],
            ...
            self.generate_range(option_key, option["min"], option["max"],
                                option["label"], value)
            ...
            self.generate_multiple_file_chooser(option_key, option["label"],
                                                value)
        elif option_type == "label":
            self.generate_label(option["label"])
        elif option_type == "mapping":
            self.generate_editable_grid(option_key,
                                        label=option["label"],
                                        value=value)
        else:
    ...
    def generate_checkbox(self, option, value=None):
        ...
        switch.connect("notify::active", self.checkbox_toggle,
                       option["option"])
    ...
    def generate_checkbox_with_callback(self, option, value=None):
        ...
        checkbox.connect("notify::active", self._on_toggle_with_callback,
                         option)
    ...
    def _on_toggle_with_callback(self, widget, _gparam, option):
        ...
        if widget.get_active() == callback_on or callback_on is None:
            AsyncCall(callback, self._on_callback_finished, widget, option,
                      self.config)
        else:
            self.option_changed(widget, option_name, widget.get_active())
    ...
    def generate_searchable_combobox(self, option_name, choice_func, label,
                                     value, default):
        """Generate a searchable combo box"""
        combobox = SearchableCombobox(choice_func, value or default)
        combobox.connect("changed", self.on_searchable_entry_changed,
                         option_name)
    ...
    def generate_editable_grid(self, option_name, label, value=None):
        ...
        try:
            ...
            logger.error("Invalid value of type %s passed to grid widget: %s",
                         type(value), value)
            value = {}
        label = Label(label)
    ...
    def on_add_files_clicked(self, _widget, option_name, value):
        ...
        dialog.set_current_folder(first_file_dir or self.game.directory
    ...
    def set_style_property(property_, value, wrapper):
        ...
        style_provider.load_from_data("GtkHBox {{{}: {};}}".format(
            property_, value).encode())
